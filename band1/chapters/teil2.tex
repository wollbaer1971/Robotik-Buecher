...
\texttt{/proc/net/dev} ist eine Datei. Viele Ressourcen werden unter Linux als Dateien dargestellt. Geräte, Pipes und zahlreiche Kernel-Schnittstellen lassen sich über das Dateisystem ansprechen.
\end{haubeBox}
...
Ein \texttt{rm -rf /} löscht das gesamte System ohne Rückfrage. Dieser Befehl zerstört das komplette System und sollte niemals verwendet werden.
\end{merksatzBox}
...
\begin{robotikBox}
\texttt{ros2 node list} zeigt alle aktiven ROS2-Nodes. \texttt{ros2 lifecycle set /sensor_node shutdown} beendet einen Node sauber. Bei hängendem System:
\begin{lstlisting}[style=bash]
pkill -f ros2
\end{lstlisting}
